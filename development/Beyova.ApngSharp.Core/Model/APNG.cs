...
using System.IO;
using Beyova.ApngSharp.Core.Constant;
...
namespace Beyova.ApngSharp.Core
{
    public class APNG
    {
        ...
        public APNG(byte[] fileBytes)
        {
            ...
            {
                throw new InvalidDataException("File signature incorrect.");
            }

            // Read IHDR chunk.
            IHDRChunk = new IHDRChunk(ms);
            if (IHDRChunk.ChunkType != ChunkTypes.IHDR)
            {
                throw new InvalidDataException("IHDR chunk must located before any other chunks.");
            }
            ...
            do
            {
                ...
                switch (chunk.ChunkType)
                {
                    case ChunkTypes.IHDR:
                        throw new InvalidOperationException("Only single IHDR is allowed.");
                        break;

                    case ChunkTypes.acTL:
                        if (IsSimplePNG)
                            throw new InvalidDataException("acTL chunk must located before any IDAT and fdAT");
